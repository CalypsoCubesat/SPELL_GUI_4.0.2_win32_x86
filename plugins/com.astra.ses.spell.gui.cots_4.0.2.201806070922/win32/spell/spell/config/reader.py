...
DESCRIPTION
    Definition of the CFG singleton class (ConfigReaderClass), which provides
    access to the XML configuration file contents
...
class ConfigReaderClass(object):
        ...
        try:
            ...
            for section in CONFIG_SECTIONS:
                if section == DATABASES:
                    LOG("Loading database definitions", level = LOG_CNFG)
                    self.loadDatabases()
                else:
            ...
            for property in node.childNodes:
                if property.nodeType == Node.ELEMENT_NODE:
                    for child in property.childNodes:
                        if child.nodeType == Node.TEXT_NODE:
                            path = str(child.data)
                            config_dir = os.getenv("SPELL_CONFIG", self.getHome() + os.sep + 'config')
    ...
    def loadDatabases(self):
        count = 0
        for node in self.document.getElementsByTagName(DATABASES):
            if count > 0: break
            count = count + 1
            
            self.properties[DATABASES] = PropertyList()
            ...
            for property in node.childNodes:
                if property.nodeType == Node.ELEMENT_NODE:
                    ...
                    for child in property.childNodes:
                        if child.nodeType == Node.TEXT_NODE:
                            path = str(child.data)
                    LOG("   - DB type: " + str(name), level = LOG_CNFG)
                    self.properties[DATABASES][name] = [type,path,ext]
    ...
    def getLocationType(self, location):
        return self.properties[DATABASES][location][0]

    #==========================================================================
    def getLocationPath(self, location):
        return self.properties[DATABASES][location][1]

    #==========================================================================
    def getLocationExt(self, location):
        return self.properties[DATABASES][location][2]
    ...
    def getUserDataDir(self):
        data = os.getenv("SPELL_DATA", self.getHome() + os.sep + "data")
    ...
    def getRuntimeDir(self):
        data = os.getenv("SPELL_SYS_DATA", self.getHome() + os.sep + "data")
        if data is None or len(data)=="":
