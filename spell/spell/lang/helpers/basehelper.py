...
PACKAGE 
    spell.lang.helpers.basehelper 
FILE
    basehelper.py
    
DESCRIPTION
    Base helper for wrapper functions. It implements the basic functionality
    of all language primitives, which consists on:
        1 - Configuring the operation
        2 - Carry out the operation
        3 - In case of failure, prompt the user for action, depending
            on the desired behavior (configured by means of cfg dictionaries)
...
class WrapperHelper(Configurable):
    ...
    def __init__(self, interface = None ):
        ...
        self._aborted = False
        self._config = self._getDefaults()
    ...
    def configure(self, *args, **kargs ):
        self._config = self.buildConfig(args, kargs, self._getDefaults(), 
        {HandleError:True,GiveChoice:False,Notify:True,PromptUser:True,AllowInterrupt:False})
        env = REGISTRY['EXEC'].getEnvironment()
        #self.checkConfig(env,locals())
    ...
    #===========================================================================
    def _startWait(self, checkCallback = None, period = 0.25 ):
        self._exec.startWait(checkCallback, period)
    ...
    #===========================================================================
    def _finishWait(self):
        self._exec.finishWait()
    ...
    def execute(self, *args, **kargs ):
        ...
        try:
            ...
            # Carry out the pre-operation and catch possible syntax errors
            #LOG("Pre-operation", level = LOG_LANG )
            try:
                # Preliminary operations
                self._doPreOperation(*args,**kargs)
            except SyntaxException,ex:
                LOG("Caught syntax exception in operation")
                self._doError(ex.message)
                #self.__unlock()
                raise SyntaxException("Syntax error in procedure: " + ex.message, ex.reason)
    ...
    def _handleException(self, exception, options = None ):
        ...
        # Perform an action depending on the prompt answer
    ...
    def _doAction(self, action):
        ...
        if action == ACTION_ABORT:
            ...
            self._notifyOpStatus( NOTIF_STATUS_PR, "Resending")
            return self._doResend()
        elif action == ACTION_REPEAT:
            self._notifyOpStatus( NOTIF_STATUS_PR, "Repeating")
            return self._doRepeat()
        elif action == ACTION_RECHECK:
            self._notifyOpStatus( NOTIF_STATUS_PR, "Rechecking")
            return self._doRecheck()
        elif action == ACTION_SKIP:
            self._notifyOpStatus( NOTIF_STATUS_SP, "Skipped")
            return self._doSkip()       
        elif action == ACTION_NOACTION:
            return [False,True]       
        elif action == ACTION_CANCEL:
            self._notifyOpStatus( NOTIF_STATUS_SP, "Cancelled")
    ...
    #===========================================================================
    def _doError(self, message ):
        self._notifyOpStatus( NOTIF_STATUS_FL, "Syntax Error")
