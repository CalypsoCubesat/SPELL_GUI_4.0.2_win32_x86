...
PACKAGE 
    spell.lib.adapter.tc
FILE
    tc.py
    
DESCRIPTION
    TC interface definition. Provides functions to send telecommands.
...
class TcInterface(Configurable,Interface):
    ...
    def __processTcList(self, tcList, useConfig):
        ...
        if useConfig.has_key(Block) and useConfig.get(Block):
            ...
            # Confirm if required
            if useConfig.has_key(Confirm) and useConfig.get(Confirm):
                LOG("Confirm execution")
                self._confirmExecution("TC block")
            self._checkCriticalCommands(tcList, useConfig)
            # Send commands as a block
            result = self._sendBlock( tcList, useConfig )
            return result
        else:
            # Send single command
            if len(tcList)==1:
                ...
                if useConfig.has_key(Confirm) and useConfig.get(Confirm):
                    LOG("Confirm execution")
                    self._confirmExecution(tcitem.name())
                self._checkCriticalCommands(tcList, useConfig)
                LOG("Sending " + tcitem.name())
                LOG("Item configuration:\n\n" + repr(tcitem.getConfig()) + "\n")
                self.__lastStatus = None
                self.__lastElement = None
                result = self._sendCommand( tcitem, useConfig )
                return result
            
            # Send list of commands one by one
            else:
                LOG("Send command group one by one")
                # Confirm if required
                if useConfig.has_key(Confirm) and useConfig.get(Confirm):
                    LOG("Confirm execution")
                    self._confirmExecution("TC list")
                self._checkCriticalCommands(tcList, useConfig)
                result = self._sendList(tcList, useConfig)
                return result
    ...
    def __getitem__(self, key):
        ...
        REGISTRY['CIF'].notify( NOTIF_TYPE_VAL, mnemonic, "created", "SUCCESS", "")
        return item
    ...
    #==========================================================================
    def _confirmExecution(self, tcItemName, msg = None):
        if msg is None:
            msg = "Please confirm execution of " + tcItemName
        try:
            confirmed = REGISTRY['CIF'].prompt(msg, config = {Type:OK_CANCEL})
        except:
            confirmed = False
        if not confirmed:
